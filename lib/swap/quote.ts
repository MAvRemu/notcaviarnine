/**
 * Aggregator swap quotes — split from the token-list module so the /api/swap/quote route can run
 * on the Edge runtime (no next/cache import): near-zero cold start, executed close to the user
 * and to Astrolescent's EU-fronted API. See lib/swap/astrolescent.ts for the token list.
 */
...
export function partnerUrl(path: string) {
