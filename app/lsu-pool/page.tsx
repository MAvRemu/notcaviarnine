...
import { getLsuComposition } from '@/lib/lsupool/composition';
import { LsuActions, RefreshPricesButton } from '@/components/lsu/lsu-actions';
import { CompositionTable } from '@/components/lsu/composition-table';
...
export default async function LsuPoolPage() {
  const live = isLive('lsu-pool');
  const [snap, composition] = await Promise.all([
    getPoolSnapshot().catch(() => null),
    live ? getLsuComposition().catch(() => null) : Promise.resolve(null),
  ]);
  const s = snap?.state;
  ...
  return (
    <ProductShell>
      <main className="mx-auto max-w-6xl space-y-6 px-4 py-6 sm:px-6">
        ...
        {!live && <ComingSoon product={productById('lsu-pool')} />}
        ...
        {live && composition ? (
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
            <LsuActions rows={composition.rows} requireActive={composition.requireActive} nav={s?.nav ?? '0'} />
            <div className="space-y-6">
              <CompositionTable rows={composition.rows} requireActive={composition.requireActive} totalXrd={Number(s?.lsuPoolValuationXrd ?? 0)} />
              <div className="flex flex-wrap items-center gap-3 text-xs text-muted">
                <RefreshPricesButton />
                <span>Prices refresh automatically with pool activity; anyone can trigger one.</span>
                <a className="hover:text-ink" href={dashboardUrl(ADDRESSES.lsuPool)} target="_blank" rel="noreferrer">On ledger ↗</a>
              </div>
            </div>
          </div>
        ) : (
